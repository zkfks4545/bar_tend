...
| 작업 ID | DATA-001 |
| 작업자 | GPT-5 Codex |
| 작업 내용 | 별도 DB 분화에 앞서 현재 단일 칵테일 목록에 활용도가 높은 IBA 클래식 25종을 추가함. |
| 주요 변경 사항 | 다이키리, 맨해튼, 위스키 사워, 사이드카, 프렌치 75, 에비에이션, 라스트 워드, 불바디에, 아메리카노, 쿠바 리브레, 모스코 뮬, 블러디 메리, 카이피리냐, 피스코 사워, 코스모폴리탄, 에스프레소 마티니, 벨리니, 스프리츠, 마이 타이, 피냐 콜라다, 싱가포르 슬링, 존 콜린스, 민트 줄렙, 라모스 피즈, 클로버 클럽 추가. 각 항목에 베이스, 계량 레시피, 한국어 설명, 추천용 맛 특성값을 기록함. `Espresso Martini`가 짧은 이름인 `Martini`로 검색되는 결함을 발견해 가장 긴 일치 이름을 우선하도록 수정함. |
| 현재 목록 | 클래식 30종 + 시그니처 2종, 총 32종. ID 및 영문 이름 중복 없음. |
| 수정 파일 | `bar_tend/src/data/cocktail-db.json`, `bar_tend/src/lib/cocktails/database.ts`, `bar_tend/src/lib/cocktails/database.test.ts`, `mission_control/WORK_LOG.md` |
| 검증 | JSON 파싱 및 중복 검사, `npm.cmd run lint`, `npm.cmd run check`, `npm.cmd test` 30개, `npm.cmd run build`, `git diff --check` 통과 |
| 후속 작업 | 필요 시 남은 IBA 공식 칵테일을 같은 목록 계약으로 추가하고, 목록 규모와 운영 요구가 확정된 뒤 DB 분화를 검토 |
