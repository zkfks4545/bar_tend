...
| 범위 | `intent`, `entities`, `statePatch`, `action`, `responseTemplateId`, `confidence` 계약과 스키마 검증, 규칙 엔진 복구 경로 |
| 완료 조건 | 규칙 엔진과 선택적 LLM이 동일 계약을 사용하고, 잘못된 JSON은 상태를 변경하지 않으며, 대화 템플릿만으로 핵심 흐름을 완료 |
