...
| 상태 | DOING |
| 예상 | 3~5일 |
| 목적 | 실제 WebLLM으로 Qwen·Gemma 계열 모델을 로드하고 다운로드 크기, 메모리, 첫 토큰 시간, 생성 속도를 측정한다 |
| 완료 조건 | 각 후보별 실행 결과 기록 (모델 ID, 다운로드 크기, 메모리, 첫 토큰, 토큰/초) |
| 의존 | RST-601 Worker 인프라 필요. RST-603 평가 세트와 채점기는 완료됨 |
| 하위 작업 | **RST-602-A (완료)**: 평가 실행기 `src/lib/webllm/evaluation.ts` 구축. 브라우저 콘솔 `__evaluateModels()`로 실행. 로드 시간·TTFT·Tok/s·E2E·Karua 16케이스 하드 실패율 측정. |
| 하위 작업 | **RST-602-B (TODO)**: 실제 브라우저(WebGPU)에서 후보 모델 3종 평가 실행 및 결과 기록 |

### RST-603: 카루아 한국어 평가 세트와 모델 선정

| 항목 | 내용 |
|---|---|
| 상태 | TODO (RST-603-A 평가 세트/채점기 완료) |
| 예상 | 2~3일 |
| 목적 | RST-602 실행 결과를 `WEBLLM_EVALUATION.md` 채점표에 기록하고 선정 기준에 따라 기본 모델 결정 |
| 완료 조건 | 후보별 결과 기록, 최종 기본 모델 결정 및 문서화 |
| 의존 | RST-602 완료 후 가능 |
...
| 의존 | RST-602(모델 평가) 완료 후 모델 선택이 끝나야 연결 가능 |
