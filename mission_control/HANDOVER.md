...
MVP 이후 논의 후보로 `DISC-001`을 기록했다. 대화 의미와 행동을 JSON 계약으로 고정하는 `DLG-801`, 미등록 칵테일 이름을 발견 상태로 저장하는 `DATA-801`, 외부 API·AI로 출처 후보를 보강하는 `DATA-802`는 승인 전 구현하지 않는다. JSON에는 대사 전문을 쌓지 않고 최소 의미 계약만 유지하며, 검색 API는 사실 수집, 코드는 상태·행동 결정, WebLLM은 검증된 내용의 말투 포장만 담당한다.
