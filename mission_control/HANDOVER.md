...
> 최종 갱신일: 2026-06-12 (RST-602-F RP 평가 계약 보완)
...
`RST-402`와 `RST-501`은 완료되었다. 적응형 질문은 자유 입력·숫자 입력·선택 버튼을 지원하며, 카루아에게 맡기기와 추천 취소, 활성 신호 제거를 제공한다. 활성 신호를 제거하면 남은 상태와 과거 질문 답변을 기준으로 후보군을 처음부터 재계산한다.
